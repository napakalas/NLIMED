...
class Annotator(GeneralNLIMED, GeneralNLP):
    ...
    def __init__(self, **settings):
        ...
        self.innerMlt = [settings['alpha'], settings['beta'],
                         settings['gamma'], settings['delta']]
        ...
        self.inv_index = self._loadJson(
            'indexes', self.repository + '_inv_index')
        self.inv_index_onto = self._loadJson(
            'indexes', self.repository + '_inv_index_onto')
        self.pred_inv_index = self._loadJson(
            'indexes', self.repository + '_pred_inv_index')
        self.pred_inv_index_onto = self._loadJson(
            'indexes', self.repository + '_pred_inv_index_onto')
        if settings['repo'] == 'pmr':
            self.idx_id_object = self._loadJson(
                'indexes', self.repository + '_idx_id_object')
            idx_id_sbj = self._loadJson(
        ...
        self.avgLengthOntos = list(
            map(mean, zip(*(self.inv_index_onto.values()))))
        self._brackets = {
    ...
    def __getTf(self, app, ontoPhraseLength, dependencyLevel, queryPhraseLength, type):
        ...
        if self.tfMode in [1, 3]:
            ...
            # similarity = app \
            # / (ontoPhraseLength+math.log(max(1, (queryPhraseLength/ontoPhraseLength)), logBase)) \
            # * ontoPhraseLength * math.log(max(K1, ontoPhraseLength+K1-dependencyLevel), logBase) \
            # / math.log(math.factorial(ontoPhraseLength+K2), logBase)

            K = 1
            similarity = app \
                * math.log(K + dependencyLevel, logBase) \
                / (1 + math.log(max(1, (queryPhraseLength/ontoPhraseLength)), logBase)) \
                / math.log(math.factorial(ontoPhraseLength+K), logBase)
        ...
        for token in deepDependency:
            if token in self.pred_inv_index:
                ...
                for predId, val in self.pred_inv_index[token].items():
                    ...
                    # special in predicate, minus 1, no parent label
                    for i in range(len(self.innerMlt)-1):
                        # dependencyLevel = max(
                        dependencyLevel = min(
                            [val[i+len(self.innerMlt)], deepDependency[token]])
                        candidateVal += self.innerMlt[i] * self.__getTf(
                            val[i], self.pred_inv_index_onto[predId][i], dependencyLevel, queryPhraseLength, 1) * idf
        ...
        for token in deepDependency:
            if token in self.inv_index:
                for oboId, val in self.inv_index[token].items():
                    tokenOboWeight = 0
                    idf = self.__getIdf(
                        len(self.inv_index[token]), self.totObject * self.inv_index_onto[oboId][descPos])
                    for i in range(len(self.innerMlt)):
                        # dependencyLevel = max(
                        dependencyLevel = min(
                            [val[i+len(self.innerMlt)], deepDependency[token]])
                        if self.inv_index_onto[oboId][i] > 0:
                            additionalVal = self.innerMlt[i] * self.__getTf(
                                val[i], self.inv_index_onto[oboId][i], dependencyLevel, queryPhraseLength, i)
                    ...
                    tokenOboWeight += self.theta * (val[-2]) / (1+math.log(queryPhraseLength)) / (1+math.log(
                        1+self.inv_index_onto[oboId][-1])) * (1+math.log(1+self.inv_index_onto[oboId][-2]))
    ...
    def _getObos(self, phrases):
        ...
        sortedPositions = collections.OrderedDict(
            sorted(hasAnnotated.items(), key=get_keylength, reverse=True))
    ...
    def _initHyperparam(self):
        ...
        settings = list(itertools.product(
            *[alphas, betas, gammas, deltas, thetas]))

        #normalise setting
        newSettings = []
        for setting in settings:
            mlt = max(alphas) / setting[0]
            newSettings += [tuple([round(mlt*setting[i], 2 if i == 4 else 1)
    ...
    def _loadPhrasesFromDataTrain(self, dataTrain, multipliers, classToUri):
        ...
        for key, queryTrain in dataTrain.items():
            ...
            for classId in queryTrain['annotation']:
                if classId not in classToUri:
                    classToUri[classId] = self._getURICode(classId)
            ...
            for pos, phrase in dictPhrases.items():
                ...
                if phrase not in phrases:
                    ...
                    for mltPos, multiplier in enumerate(multipliers):
                        ...
                        candidates = self._getPossibleObo(
                            self.tokenise(phrase))['candidate']
                        phrases[phrase][mltPos]['classes'] = list(
                            map(itemgetter(0), candidates))
    ...
    def _getPhraseSettingWeight(self, phrases, precAt, settings, cutoffs, classToUri):
        ...
        maxSettingIdx = len(settings[0])-1
            ...
            for setting in settings:
                ...
                for mltPos, mltVal in enumerate(setting):
                    for classId, featVal in zip(phraseWeights[mltPos]['classes'], phraseWeights[mltPos][mltVal]):
                        tmpClasses[classId] = tmpClasses[classId] + \
                            featVal if classId in tmpClasses else featVal
                        # remove classId which values lower than minimum cutoff
                        if mltPos == maxSettingIdx and tmpClasses[classId] < minCutOff:
                            del tmpClasses[classId]
                # sort and remove duplicates (e.g CHEBI_15378 and CHEBI:15378) and store in standard id (e.g chebi15378)
                tmpClasses = {k: v for k, v in sorted(
                    tmpClasses.items(), key=itemgetter(1), reverse=True)}
                tmp = {}
                if precAt == -1:
                    precAt = len(tmpClasses)
    ...
    def _getSettingStat(setting, posSetting, stats, totPredictCorrect, totPredictLen, totClassData, queryCorrect, numOfDataTrain, totPredictCorrectPhrase, totPredictCorrectWord):
        ...
        def getLocalStat(prediction, dataType):
            for dataLen in prediction.copy():
                ...
                localPrecision = sum(
                    transpose[0]) / sum(transpose[1]) if sum(transpose[1]) > 0 else 0
                localRecall = sum(
                    transpose[0]) / sum(transpose[2]) if sum(transpose[2]) > 0 else 0
                localFMeasure = 2 * (localPrecision * localRecall) / (
                    localPrecision + localRecall) if localPrecision + localRecall > 0 else 0
                prediction[dataLen] = [round(localPrecision, 3), round(
                    localRecall, 3), round(localFMeasure, 3)]
    ...
    def _annotatePhrases(self, queryTrain, phrases, setting):
        ...
        for pos, phrase in queryTrain['phrases'].items():
            if len(phrases[phrase][setting]) > 0:
                hasAnnotated[pos] = (list(phrases[phrase][setting].keys(
                )), phrases[phrase][setting][list(phrases[phrase][setting].keys())[0]])
    ...
    def _getHyperparamStat(self, phrases, dataTrain):
        ...
        for count, setting in enumerate(newSettings):
            ...
            for key, queryTrain in dataTrain.items():
                hasAnnotated = self._annotatePhrases(
                    queryTrain, phrases, setting)
                ...
                if dataLen not in totPredictCorrectPhrase:
                    totPredictCorrectPhrase[dataLen] = []
                totPredictCorrectPhrase[dataLen] += [
                    (predictCorrect, predictLen, dataLen)]
                if len(queryTrain['query'].split()) not in totPredictCorrectWord:
                    totPredictCorrectWord[len(
                        queryTrain['query'].split())] = []
                totPredictCorrectWord[len(queryTrain['query'].split(
                ))] += [(predictCorrect, predictLen, dataLen)]
            ...
            print(count if count % 10000 == 0 else '.' if count % 500 == 0 else count if (
                len(newSettings)-1) == count else '', end='', flush=True)
            Annotator._getSettingStat(setting, count, stats, totPredictCorrect, totPredictLen, totClassData, queryCorrect, len(
                dataTrain), totPredictCorrectPhrase, totPredictCorrectWord)
    ...
    def hyperparam(self, datatTrainFile, precAt, isVerbose=True):
        ...
        """
        print("Hyperparam setup, repo:%s, precAt:%d" %
              (self.repository, precAt), flush=True)
        ...
        if not os.path.exists(datatTrainFile):
            print("Error, test data file is not found")
            return {}
        # load data train and organised all possible noun phrase
        with open(datatTrainFile, 'r') as fp:
            dataTrain = json.load(fp)
        # load data train and organised all possible noun phrase
        phrases = self._loadPhrasesFromDataTrain(
            dataTrain, multipliers, classToUri)
        print('\nLoading data train and phrases detection, done ...., %fs' %
              (time.time()-second))
        print('Number of phrases:%d' % len(phrases))
        second = time.time()
        # get weight of all phrases for each setting
        self._getPhraseSettingWeight(
            phrases, precAt, settings, cutoffs, classToUri)
        print('\nCalculate all phrases for all settings, done ...., %fs' %
              (time.time()-second))
        second = time.time()
    ...
    def _annotateAucPhrases(self, queryTrain, phrases, setting):
        ...
        for pos, phrase in queryTrain['phrases'].items():
            if len(phrases[phrase][setting]) > 0:
                hasAnnotated[pos] = (phrases[phrase][setting], list(
                    phrases[phrase][setting].values())[0])
    ...
    def _getAucStat(self, phrases, dataTrain):
        ...
        totPhraseTrain = sum([len(qt['annotation'])
                              for qt in dataTrain.values()])
        for count, setting in enumerate(settings):
            levelCorrect, levelReturn = {}, {}
            for key, queryTrain in dataTrain.items():
                hasAnnotated = self._annotateAucPhrases(
                    queryTrain, phrases, setting)
                cutoff = 0
                while True:
                    newHasAnnotated = []
                    ...
                    precAt = 1
                    while True:
                        setReturn = {k for phraseAnn in hasAnnotated for k in list(phraseAnn.keys())[
                            :precAt]}
                        if precAt > 5:
                            break
                        if (cutoff, precAt) not in levelCorrect:
                            levelCorrect[(cutoff, precAt)] = 0
                            levelReturn[(cutoff, precAt)] = 0
                        levelCorrect[(cutoff, precAt)] += len(setReturn &
                                                              set(queryTrain['annotation']))
                        levelReturn[(cutoff, precAt)] += len(setReturn)
                        precAt += 1

                    detectedClasses = [
                        k for phraseAnn in hasAnnotated for k, v in phraseAnn.items()]
            ...
            recalls, precisions, cutoffs = list(
                recalls), list(precisions), list(cutoffs)
            ...
            if idx70 < len(recalls)-1:
                ...
                addPrec = precisions[idx70] + \
                    ((precisions[idx70+1]-precisions[idx70])*prop)
            ...
            if idx50 < len(recalls)-1:
                ...
                prop = (0.5043478260869565 -
                        recalls[idx50])/(recalls[idx50+1]-recalls[idx50])
                addPrec = precisions[idx50] + \
                    ((precisions[idx50+1]-precisions[idx50])*prop)
            ...
            # store to stats
            stats['precisions'][count] = [
                round(precision, 3) for precision in precisions]
            stats['recalls'][count] = [
                round(recalls, 3) for recalls in recalls]
            ...
            for phrase in phrases.values():
                del phrase[setting]
            print(count if count % 10000 == 0 else '.' if count % 500 == 0 else count if (
                len(settings)-1) == count else '', end='', flush=True)
    ...
    def auc(self, datatTrainFile):
        ...
        # initialised multiplier and settings:
        hSettings = self._initHyperparam()
        multipliers, settings, cutoffs = hSettings['multipliers'], hSettings['settings'], {
            0}  # hSettings['cutoffs']
        classToUri = {}
        # initial to calculate execution time
        import time
        ...
        # get weight of all phrases for each setting
        self._getPhraseSettingWeight(
            phrases, -1, settings, cutoffs, classToUri)
        print('\nCalculate all phrases for all settings, done ...., %fs' %
              (time.time()-second))
...
class CoreNLPAnnotator(Annotator):
    ...
    def __init__(self, **settings):
        ...
        try:
            import urllib3
            proxy = urllib3.ProxyManager(self.corenlp_ep)
            resp = proxy.request('GET', self.corenlp_ep)
            content = resp.read()
            resp.release_conn()
            # import urllib
            ...
            try:
                ...
                client.start()
                import time
                # starting usually needs time, so wait until ready
                time.sleep(10)
                print('Starting server is succeed')
            except:
                print(
                    'CoreNLP server cannot be started, use another parser {Benepar,ncbo,Stanza, xStanza}')
    ...
    def _getPhrases(self, query):
        ...
        for subtree in tree.subtrees(filter=lambda t: t.label() in ['NP', 'ADVP', 'FRAG']):
            leaves = [leaf if leaf not in self._brackets else self._brackets[leaf]
...
class OBOLIBAnnotator(GeneralNLIMED, GeneralNLP):
    ...
    def hyperparam(self, datatTrainFile, precAt, isVerbose=True):
        ...
        if os.path.exists(datatTrainFile):
            ...
            totPCorrect = 0
            totPLen = 0  # p for predict
            queryCorrect = 0
            totClassData = 0
            totPCorrectPhrase = {}
            totPCorrectWord = {}  # for specific number of phrases or words

            for counter, (key, queryTrain) in enumerate(dataTrain.items()):
                ann = self.annotate(query=queryTrain['query'])
                phrases = ann['phrases']
                instances = [self._getURICode(instance)
                             for instance in ann['result'][0][0]]
                queryTrain['annotation'] = [self._getURICode(
                    ann) for ann in queryTrain['annotation']]
                ...
                if dataLen not in totPCorrectPhrase:
                    totPCorrectPhrase[dataLen] = []
                totPCorrectPhrase[dataLen] += [(pCorrect, pLen, dataLen)]

                if len(queryTrain['query'].split()) not in totPCorrectWord:
                    totPCorrectWord[len(queryTrain['query'].split())] = []
                totPCorrectWord[len(queryTrain['query'].split())
                                ] += [(pCorrect, pLen, dataLen)]

            Annotator._getSettingStat([], 0, stats, totPCorrect, totPLen, totClassData, queryCorrect, len(
                dataTrain), totPCorrectPhrase, totPCorrectWord)

            print('\nCalculate all phrases, done ...., %fs' %
                  (time.time()-second))
            return stats
        else:
            print("Error, test data file is not found")
...
class StanzaAnnotator(Annotator):
        ...
        for doc in self.__docs:
            for sentence in doc.sentences:
                for entity in sentence.entities:
                    if entity.type == 'TEST':
                        continue  # exclude TEST type entity
                    if entity.start_char not in dictPhrases:
                        ...
                        if entity.type not in dictPhrases[entity.start_char]['type']:
                            dictPhrases[entity.start_char]['type'] += '|' + \
                                entity.type
                    elif dictPhrases[entity.start_char]['end_char'] < entity.end_char:
                        dictPhrases[entity.start_char] = entity.to_dict()
    ...
    def _getPhrases(self, query):
        ...
        keys = list(dictPhrases.copy().keys())
        currPos = 0
...
class MixedAnnotator(StanzaAnnotator):
    ...
    def __getMainHeadFromEntity(self, entity):
        ...
        for wordId in wordIds.copy():
            if wordId in headIds:
                headIds.remove(wordId)
    ...
    def __getContexts(self, entBorders, entity, sentence):
        ...
        if entBorders[0] != None:
            ...
            leftCtx = ' '.join([sentence.words[i].text for i in range(
                entBorders[0]-1, entity.words[0].id-1)])
            context += [{'obo': leftCtx + ' ' + entity.type.replace('_', ' ') if len(
                leftCtx.strip()) > 0 else '', 'pred': leftCtx, 'start_char': leftStart, 'end_char': leftEnd}]
        if entBorders[1] != None:
            rightStart = entity.end_char + 1
            rightEnd = sentence.tokens[entBorders[1]-1].end_char
            rightCtx = ' '.join([sentence.words[i].text for i in range(
                entity.words[-1].id, entBorders[1])])
            context += [{'obo': entity.type.replace('_', ' ') + ' ' + rightCtx if len(rightCtx.strip(
            )) > 0 else '', 'pred': rightCtx, 'start_char': rightStart, 'end_char': rightEnd}]
        return context
    ...
    def __getContextOnto(self, context):
        ...
        for ctx in context:
            ...
            if ctx['pred'] != '':
                predicates = self._getPossibleOntoPredicate(
                    self.tokenise(ctx['pred']))
            else:
        ...
        for doc in self.__docs:
            for sentence in doc.sentences:
                ...
                for entity in sentence.entities:
                    ...
                    entBorders = self.__getBorderOfEntity(
                        mainAndHead[0], stcBorders)
        ...
        for k, v in dictPhrases.items():
            for context in v['context']:
                for ctType in ['obo', 'pred']:
                    for eText, eType in dictEntities.items():
                        context[ctType] = context[ctType].replace(
                            eText, min(eType, key=len))
                    context[ctType] = '' if context[ctType].isupper(
                    ) else context[ctType].replace('_', ' ')
        # print(dictPhrases,'\n')
        return dictPhrases
    ...
    def annotate(self, query):
        ...
        for key, phrase in dictPhrases.items():
            ...
            if len(possibleObo['candidate']) > 0:
                ...
                predicates += [[pred for preds in contextOnto['predicate']
                                for pred in preds['candidate']]]
                entities[key]['predicate'] = [
                    pred for preds in contextOnto['predicate'] for pred in preds['candidate']]
                # 4. Specify contex ontology instances as additional ontologies
    ...
    def _loadPhrasesFromDataTrain(self, dataTrain, multipliers, classToUri):
        ...
        for key, queryTrain in dataTrain.items():
            ...
            for pos, phrase in dictPhrases.items():
                ...
                phraseAndContexts = [phrase]+[context[key]
                                              for context in contexts for key in context.keys() if key in ['obo', 'pred']]
                for phrase in phraseAndContexts:
                    if phrase not in phrases:
                        ...
                        for mltPos, multiplier in enumerate(multipliers):
                            ...
                            for val in multiplier:
                                modifiedValue = [
                                    val*init for init in initialValue]
                                phrases[phrase][mltPos][val] = modifiedValue
    ...
    def _annotatePhrases(self, queryTrain, phrases, setting):
        ...
        for pos, phrase in queryTrain['phrases'].items():
            if len(phrases[phrase][setting]) > 0:
                hasAnnotated[pos] = (list(phrases[phrase][setting].keys()), max(
                    phrases[phrase][setting].values()))
            ...
            for context in contexts:
                ...
                if len(phrases[context['obo']][setting]) > 0:
                    if len(phrases[context['pred']][setting]) == 0:
                        hasAnnotated[pos] = (list(phrases[context['obo']][setting].keys()), max(
                            phrases[context['obo']][setting].values()))
