    ...
    def _getObos(self, tree):
        ...
        for subtree in tree.subtrees(filter=None):
            if subtree.label() == 'NP':
                ...
                # print(entity['phrase'], ' ', entity['candidate'], ' ', entity['maxValue'])
                ...
                if entity['phrase'] not in control:
                    ...
                    control[entity['phrase']] = pos
                    childParent[pos] = posParent
        # print(len(hasAnnotated), ' ',hasAnnotated,'\n')
        # print(len(childParent), ' ',childParent,'\n')
        ...
        listOfPos = list(childParent.keys())
        listOfPos.reverse()
        for pos in listOfPos:
            if pos in hasAnnotated:
                parentPos = childParent[pos]
                copyAnnotatedKeys = list(copy.deepcopy(hasAnnotated).keys())
                copyAnnotatedKeys.reverse()
                for key in copyAnnotatedKeys:
                    if len(parentPos) >= len(key):
                        if parentPos[:len(key)] == key:
                            if hasAnnotated[pos]['maxValue'] <= hasAnnotated[key]['maxValue']:
