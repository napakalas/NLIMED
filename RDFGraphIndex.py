...
class IndexSPARQL(GeneralNLIMED):
    ...
    def __buildIndexBM(self):
        ...
        def getAllRdfs():
            ...
            self._saveBinaryInteger(cellmlPaths, 'BM_rdfPaths')
            self._dumpJson(dictMainSubjects, 'BM_subject.json')
            self._dumpJson(dictObjects, 'BM_object.json')
            # modify trac so it store --> idTrack, [track]
            reverseDictTracks = {value: list(key)
                                 for key, value in dictTracks.items()}
            self._dumpJson(reverseDictTracks, 'BM_track.json')
            self._dumpJson(list(entityTypes), 'BM_entityTypes')
            print("Seconds since epoch =", time.time() - seconds)
            return cellmlPaths
        getAllRdfs()

    def getAllUrlContents(self):
        links = self._loadJson('listOfLinks.json')
        data = self._loadJson('cellMlData.json')
        for link, isUpdate in links.items():
            if bool(isUpdate):
                text = requests.get(link).text
                data[link] = text
        print(len(data))
        self._dumpJson(data, 'cellMlData.json')
    ...
    def getAllRdfs(self):
        cellMlData = self._loadJson('cellMlData.json')
        cellmlPaths = []
        ...
        self._dumpJson(cellmlPaths, 'rdfPaths.json')
        return cellmlPaths
    ...
    def createIndexPrefix(self):
        ...
        data = self._loadJson('rdfPaths.json')
        index_pref_ns = {}  # {pref0:ns0, pref1:ns1, ...}
        index_ns_pref = {}  # {ns0:pref0, ns1:pref1, ...}
        list_namespaces = []
...
#         list_namespaces = list(set(list_namespaces))
        ...
        self._dumpJson(index_pref_ns, 'idx_pref_ns')
        self._dumpJson(index_ns_pref, 'idx_ns_pref')

    def createIndexPredicate(self):
        # create index of predicate
        data = self._loadJson('rdfPaths.json')
        index_ns_pref = self._loadJson('idx_ns_pref')
        ...
        self._dumpJson(index_predi_pred, 'idx_id_pred')
        # save predicate to file
        index_pref_ns = self._loadJson('idx_pref_ns')
        myPredList = []
        for myPred in list_predicate:
            myPredList += [index_pref_ns[str(myPred[0])] + myPred[1]]
        self._saveToFlatFile(myPredList, 'listOfPredicates.txt')
    ...
    def createIndexObjectAndSubject(self):
        ...
        self._dumpJson(index_oi_object, 'idx_id_object')
        self._dumpJson(index_object_oi, 'idx_object_id')
        self._dumpJson(index_si_subject, 'idx_id_subject')
        self._dumpJson(index_subject_si, 'idx_subject_id')
        # save obj to file
        self._saveToFlatFile(list_object, 'listOfObjects.txt')
        # save sbj to file
        self._saveToFlatFile(list_subject, 'listOfSubjects.txt')
    ...
    def restructureRdfPath(self):
        ...
        data = self._loadJson('rdfPaths.json')
        index_object_oi = self._loadJson('idx_object_id')
        index_subject_si = self._loadJson('idx_subject_id')
        index_ns_pref = self._loadJson('idx_ns_pref')
        index_id_pred = self._loadJson('idx_id_pred')
        index_pred_predi = {}
        ...
        self._dumpJson(restructuredPaths, 'restructureRdfPath.json')

    def createIndexTrack(self):
        # create index of track
        data = self._loadJson('restructureRdfPath.json')
        ...
        # save to index file
        self._dumpJson(index_tracki_track, 'idx_id_track')
        # save track to file
        self._saveToFlatFile(list_track, 'listOfTracks.txt')
    ...
    def fullyRestructureRdfPath(self):
        ...
        data = self._loadJson('restructureRdfPath.json')
        index_track_tracki = {}
        index_tracki_track = self._loadJson('idx_id_track')
    ...
    def createIndexObjectSubjectPair(self):
        ...
        self._dumpJson(index_obj_sbj, 'idx_obj_sbj')
        self._dumpJson(index_sbj_obj, 'idx_sbj_obj')
        tmp_index_sbjobj_tracks = []
        for key, val in index_sbjobj_tracks.items():
            tmp_index_sbjobj_tracks += [[list(key), val]]
        self._dumpJson(tmp_index_sbjobj_tracks, 'idx_sbjobj_tracks')
